...
| **4** | Interactions Compose + smoke E2E bout-en-bout | Compose UI Test + Espresso | émulateur | ⬜ à faire |
...
## Couche 4 — Interactions Compose + smoke E2E *(à faire)*

- **Où** : `app/src/androidTest/…` (émulateur).
- **Cible** : flux interactifs non capturés par le screenshot — réponse QCM (feedback couleur + liste d'erreurs), saisie édition, barre de progression conservée après revue d'erreurs, bascule nav, CRUD sélections, changement de langue (recréation d'activité), `DialogFlowController` (auto-skip fin de quiz). Plus **1 scénario E2E** : Study → lancer quiz → répondre toute la session → dialog de fin → écran Answers.
- **Outils** : `createAndroidComposeRule` + Espresso (déjà au version catalog : `espresso-core`, `compose-ui-test-junit4`).
...
- **Couche 4** entière (interactions Compose + E2E) — non commencée.
