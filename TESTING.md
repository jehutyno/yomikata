...
- **Fait** (13 tests, 5 classes) :
  ...
  - `WordListScreenInteractionTest` — saisie recherche → `onSearchQueryChanged` ; taper un mot → `onWordClick` ; étoile → `onFavoriteClick` ; mots rendus.
  - `AppLaunchSmokeTest` — la vraie `QuizzesActivity` atteint RESUMED au lancement **et après `recreate()`** (couvre le chemin de recréation config/langue qui vidait la Home).
...
- **Couche 4** : nav, quiz, sélections, liste de mots, smoke (lancement + recréation) couverts ; reste édition/IME, progression post-erreurs, création/renommage/suppression de sélection, changement de langue piloté, `DialogFlowController`, E2E complet.
