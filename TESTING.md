...
| **4** | Interactions Compose (callbacks ↔ UI) | Compose UI Test | émulateur/appareil | 🟡 nav, quiz, sélections, study, liste de mots |
...
- **Lancer** : émulateur up, puis `.\gradlew.bat connectedDebugAndroidTest` (**suite complète**)
- **Piège AGP 9.2** : les filtres `-Pandroid.testInstrumentationRunnerArguments.class=…` / `package=…` ne découvrent pas de façon fiable les classes réparties en dex secondaires (`classesN.dex`) → un filtre peut renvoyer « 0 tests » ou `ClassNotFoundException` alors que la classe est bien dans l'APK et passe en run complet. **Lancer la suite entière** (sans filtre) pour les tests d'instrumentation.
...
## Couche 4 — Interactions Compose

- **Où** : `app/src/androidTest/kotlin/com/jehutyno/yomikata/compose/` (émulateur ou appareil).
- **Lancer** : `.\gradlew.bat connectedDebugAndroidTest` (**suite entière** — cf. piège des filtres en couche 2).
- **Fait** (5 classes — nav, quiz, sélections, study, liste de mots) :
  ...
  - `SelectionsScreenInteractionTest` — taper une sélection → `onSelectionClick` ; cocher → `onSelectionChecked` ; 2 sélections rendues.
  - `StudyScreenInteractionTest` — taper une pastille de niveau → `onCategorySelected` ; taper une carte de quiz → `onQuizClick` ; noms rendus.
  - `WordListScreenInteractionTest` — saisie recherche → `onSearchQueryChanged` ; taper un mot → `onWordClick` ; rendu mots/étoile.
- **Pièges** :
  - **Geler les animations infinies** : envelopper les écrans à héros KenBurns (Study) dans `CompositionLocalProvider(LocalInspectionMode provides true)` sinon `createComposeRule` peut ne jamais devenir idle.
  - **Cibles tactiles** : cliquer une petite `IconButton` (étoile favori 13 dp, action de top bar) échoue de façon flaky sur appareil physique (« Failed to inject touch input ») → préférer une assertion de rendu, ou cliquer une zone large (ligne, chip, option).
  - **Lancement d'activité réelle non automatisé** : un smoke `ActivityScenario.launch(QuizzesActivity)` est instable (l'overlay de tutoriel d'accueil `showTutoAlways`, toujours affiché, plante pendant l'attachement de vue sous instrumentation sur certains appareils et fait **crasher tout le run**). Le lancement à froid est couvert par la couche 5 (checklist 1er lancement).
- **Reste** (backlog) : saisie édition + IME (champ `HiraganaEditText` via `AndroidView` → Espresso), barre de progression conservée après revue d'erreurs, création/renommage/suppression de sélection, changement de langue piloté depuis Settings, `DialogFlowController` (auto-skip), E2E complet Study → quiz → Answers.
...
## Baselines screenshot & rapports versionnés

### Cycle de vie d'une baseline screenshot
Les PNG de `app/src/test/screenshots/` sont la **référence « golden »**, committée dans git. Deux niveaux d'analyse, à ne pas confondre :
- **Diff pixel — automatique, à chaque run** : `verifyRoborazziDebug` rend chaque écran et le compare au baseline → build **rouge à la moindre dérive**. C'est le gate de régression.
- **Revue visuelle — ponctuelle, à l'enregistrement** : valider que la baseline est *correcte* (pas de troncature, bonnes couleurs). Fait une fois au `record`, et en **revue du diff d'image** quand une baseline change. ⚠️ Une baseline buggée passerait `verify` : la revue à l'enregistrement est le garde-fou.

**Quand l'UI change volontairement** :
1. `.\gradlew.bat recordRoborazziDebug` régénère toutes les baselines (rendu déterministe).
2. `git status app/src/test/screenshots/` → seules les baselines réellement modifiées apparaissent.
3. Relire les PNG modifiés, puis **committer les nouvelles baselines dans le même commit que le changement d'UI** → le diff d'image est revu et git ancre la baseline à la version.

**Ne PAS committer** : `app/build/outputs/roborazzi/` (images `actual`/`compare`/diff d'un run) — artefacts, déjà gitignored. **Retrouver la version d'une baseline** : `git log -1 --format="%h %ci" -- app/src/test/screenshots/home_en.png`.

### Rapports de test ancrés à une version
Ce qui est versionné (baselines, code des tests, `schemas/*.json`) est ancré par commit, gratuitement. Les **résultats d'exécution** sont des artefacts à **estampiller**. Le script `scripts/test-report.ps1` produit un rapport Markdown daté, ancré à `commit + versionName/code + device`, à partir des résultats déjà générés :
...
# 2) générer le rapport estampillé
.\scripts\test-report.ps1                     # -> build/reports/test-report-<version>-<sha>.md
```

Le rapport stampe **version, commit/branche, arbre propre/modifié, date, device, statut et compteurs par suite**, et signale si l'arbre est modifié (rapport non fiable pour une release). **Pour une release** : committer/taguer d'abord (arbre propre), lancer les suites sur ce commit, puis générer le rapport → il est ancré au tag. Quand une CI sera en place, produire ce rapport automatiquement `on: tag` et l'archiver comme artefact.

---

## Lacunes connues (backlog de test)

- **Couche 4** : interactions nav, quiz, sélections, study, liste de mots couvertes ; reste édition/IME, progression post-erreurs, création/renommage/suppression de sélection, changement de langue piloté, `DialogFlowController`, E2E complet (le lancement à froid de l'activité réelle n'est pas automatisé — overlay d'accueil fragile sous instrumentation).
