...
| `home` | `HomeFragment` | Onglet de démarrage. Dashboard Compose — hero Ken Burns (photos `pic_*` + `yomi_logo_home.png`), grille StatCards 2×2 (stats aujourd'hui), section Continue (bouton intégré), news Firebase, card Support (affichée + URL pilotées par Firebase `sponsor_available`/`sponsor_url`) |
| `content` | `ContentActivity` | Liste des mots d'une catégorie. Layout : `FrameLayout` plein écran + `ComposeView` `launch_bar` (bouton `FABBar` « Lancer le quiz » + `LaunchOptionsSheet` partagée, comme Study). Pas de Toolbar — `WordListScreen` Compose gère sa propre `TopAppBar`. Supporte mode ViewPager2 (swipe entre sélections) et mode level (filtre par niveau). |
...
**Mode sélection multiple → ajout groupé (Session wordlist-multi-selection) :** rétablit l'ajout de plusieurs mots d'un coup à une Sélection (feature perdue à la migration Compose ; l'ancien `WordsAdapter.checkMode` + `WordSelectorActionModeCallback` en `ActionMode` était du code mort RecyclerView, supprimé). **Entrée par appui long** sur une ligne (`WordListRow` en `combinedClickable`) : `WordListUiState.isSelectionMode = true` + le mot est coché. En mode sélection, la pastille de niveau devient une case à cocher (cochée = `CheckCircle` `AccentOrange` ; sinon cercle bordé), le fond de ligne coché passe `SurfaceAccent`, et les icônes ★/audio sont masquées (le tap coche/décoche via `onWordCheckToggle`). Le `TopAppBar` normal est remplacé par une **`SelectionTopBar` contextuelle** : ✕ (quitte le mode) + compteur, puis 3 actions groupées — **tout cocher/décocher** (`ic_select_multiple`, bascule selon l'état des mots filtrés visibles), **ajouter** (★ `AccentOrange`), **retirer** (`ic_unselect`). Un `BackHandler(enabled = isSelectionMode)` fait quitter le mode au retour arrière. Les actions ajouter/retirer appellent le nouveau dialog **`Fragment.showWordsSelectionDialog(wordIds, add, …)`** (`util/SelectionCreation.kt`) : liste **mono-choix** des sélections cibles (`setItems`, car l'appartenance d'un lot est hétérogène) + « nouvelle sélection » (ajout uniquement), puis boucle `addWordToSelection`/`deleteWordFromSelection` gardée par `isWordInQuiz`. **Zéro changement données/presenter/migration** — réutilise `SelectionsInterface`/`WordInQuizInterface` et les chaînes existantes (`add_to_selections`/`select_all`/`unselect_all`/`remove_from_selection`).

**Fichiers clés :**
- `ui/wordlist/WordListScreen.kt` — `WordListUiState`, `WordListScreen`, `MasteryFilterChip` (« Tous ») + `LevelFilterChip` (4 ronds couleur) + `tabToLevel`, `SelectionTopBar` (mode sélection multiple), filtrage 4 niveaux + recherche in-memory, toggle liste/grille
- `ui/word/WordListRow.kt` — ligne de mot (pastille `levelColor(Level)` cohérente aux filtres, furigana/kanji/trad, chip POS, ★ favori, audio ; `combinedClickable` appui long + rendu coché en mode sélection)
- `util/SelectionCreation.kt` — `createNewSelectionDialog` + `showWordSelectionDialog` (sélecteur multi-choix mono-mot partagé Quiz/Content/Détail) + `showWordsSelectionDialog` (ajout/retrait groupé mono-choix)
