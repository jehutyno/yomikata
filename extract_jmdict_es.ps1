# extract_jmdict_es.ps1
# Downloads JMdict (full multilingual), extracts Spanish glosses, updates asset DB.
...
$SqlPath   = Join-Path $TempDir "jmdict_es_updates.sql"
...
# Reset all spanish first to allow re-run
& $Sqlite3 $DbPath "UPDATE words SET spanish = '';"
...
# Build lookup indices (kanji→words and reading→words)
...
$esList  = [System.Collections.Generic.List[string]]::new()
$inSense = $false; $inGloss = $false; $isEs = $false
$currentText = [System.Text.StringBuilder]::new()

function Match-And-Store {
    if ($esList.Count -eq 0) { $kebList.Clear(); $rebList.Clear(); $esList.Clear(); return }
    $esStr = $esList -join '; '
    ...
    foreach ($id in $matched) {
        if (-not $results.ContainsKey($id)) { $results[$id] = $esStr }
    }
    $kebList.Clear(); $rebList.Clear(); $esList.Clear()
}
        ...
        ([System.Xml.XmlNodeType]::EndElement) {
            switch ($reader.LocalName) {
                ...
                'gloss' {
                    ...
                    $inGloss = $false; $isEs = $false; [void]$currentText.Clear()
                }
                'sense' { $inSense = $false }
            }
        }
...
foreach ($kv in $results.GetEnumerator()) {
    ...
    $sqlWriter.WriteLine("UPDATE words SET spanish = '$escaped' WHERE _id = $($kv.Key);")
}
...
# ── 5. Verify ─────────────────────────────────────────────────────────────────
$filled = & $Sqlite3 $DbPath "SELECT COUNT(*) FROM words WHERE spanish != '';"
$accent = & $Sqlite3 $DbPath "SELECT COUNT(*) FROM words WHERE spanish LIKE '%á%' OR spanish LIKE '%é%' OR spanish LIKE '%í%' OR spanish LIKE '%ó%' OR spanish LIKE '%ú%' OR spanish LIKE '%ñ%';"
Write-Host "Done. Filled: $filled / 7503 | With accents: $accent"
